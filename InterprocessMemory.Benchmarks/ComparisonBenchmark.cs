...
namespace InterprocessMemory.Benchmarks;
...
{
    private SingleProducerByteStream _spsc = null!;
    private ConcurrentMessageQueue _mpmc = null!;
    ...
    public void Setup()
    {
        _spsc = new SingleProducerByteStream("Bench_SPSC", 16 * 1024 * 1024);
        _mpmc = new ConcurrentMessageQueue("Bench_MPMC", slotCount: 1024, slotSize: 1024 + 16);
...
{
    private MemoryRegion _buffer = null!;
    ...
    public void Setup()
    {
        ...
        _buffer = new MemoryRegion("Bench_Lock", options);
        _data = new byte[1024];
        _readBuffer = new byte[1024];
...
public class ThroughputByDataSizeBenchmark
{
    ...
    public void Setup()
    {
        ...
        _buffer = new MemoryRegion("Bench_Throughput", options);
...
{
    private MemoryRegion _simdEnabled = null!;
    private MemoryRegion _simdDisabled = null!;
    private byte[] _data = null!;
    private byte[] _readBuffer = null!;
    ...
    public void Setup()
    {
        var simdOptions = new MemoryRegionOptions
        {
            Capacity = 64 * 1024 * 1024,
            EnableSimd = true
        };
        var noSimdOptions = new MemoryRegionOptions
        {
            Capacity = 64 * 1024 * 1024,
            EnableSimd = false
        };

        _simdEnabled = new MemoryRegion("Bench_SIMD", simdOptions);
        _simdDisabled = new MemoryRegion("Bench_NoSIMD", noSimdOptions);
        _data = new byte[DataSize];
        _readBuffer = new byte[DataSize];
        Random.Shared.NextBytes(_data);
    }
...
public class ChecksumOverheadBenchmark
{
    ...
    public void Setup()
    {
        ...
        _buffer = new MemoryRegion("Bench_Checksum", options);
        _data = new byte[DataSize];
        Random.Shared.NextBytes(_data);
...
{
    private ConcurrentMessageQueue _buffer = null!;
    private byte[] _message = null!;
    ...
    public void Setup()
    {
        _buffer = new ConcurrentMessageQueue("Bench_Contention", slotCount: 16384, slotSize: 128);
