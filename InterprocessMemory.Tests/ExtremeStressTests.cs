...
public class ExtremeStressTests
{
    ...
    public async Task MPMC_16Producers_16Consumers_1MillionMessages()
    {
        ...
        var consumers = Enumerable.Range(0, consumerCount).Select(consumerId => Task.Run(() =>
        {
            ...
            while (true)
            {
                ...
                {
                    if (Volatile.Read(ref producersDone) == producerCount &&
                        buffer.ApproximateCount == 0)
                        break;
                    Thread.SpinWait(1);
                }
            }
    ...
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MPMC_Burst"), slotCount: 1024, slotSize: 64);
    ...
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("SPSC_5M"), 1024 * 1024);
    ...
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("SPSC_VarSize"), 64 * 1024);
    ...
    #region MemoryRegion Extreme Tests

    [Test]
    [Timeout(30000)]
    public async Task HPBuffer_32Threads_ConcurrentLocking()
    {
        var options = new MemoryRegionOptions { Capacity = 64 * 1024 };
        using var buffer = new MemoryRegion(GetUniqueName("HP_32Thread"), options);
    ...
    public void HPBuffer_LargeDataTransfer_1GB()
    {
        ...
        using var buffer = new MemoryRegion(GetUniqueName("HP_1GB"), options);
    ...
    public void HPBuffer_ChecksumIntegrity_UnderLoad()
    {
        ...
        using var buffer = new MemoryRegion(GetUniqueName("HP_Checksum"), options);
    ...
    #region StructuredMemory Extreme Tests
    ...
    public async Task Strict_16Threads_MixedFieldAccess()
    {
        ...
        using var memory = new StructuredMemory<ExtendedTestSchema>(GetUniqueName("Strict_16T"), schema);
    ...
    public void Strict_ArrayField_LargeData()
    {
        ...
        using var memory = new StructuredMemory<LargeArraySchema>(GetUniqueName("Strict_LargeArr"), schema);
    ...
    {
        using var mpmcBuffer = new ConcurrentMessageQueue(GetUniqueName("Chaos_MPMC"), slotCount: 256, slotSize: 128);
        using var spscBuffer = new SingleProducerByteStream(GetUniqueName("Chaos_SPSC"), 64 * 1024);
        var options = new MemoryRegionOptions { Capacity = 16 * 1024 };
        using var hpBuffer = new MemoryRegion(GetUniqueName("Chaos_HP"), options);
    ...
    public async Task Chaos_RapidCreateDestroy()
    {
        ...
        await Task.Run(() =>
        {
            for (int cycle = 0; cycle < cycles; cycle++)
            {
                try
                {
                    using var buffer = new ConcurrentMessageQueue(
    ...
    {
        var buffers = new List<ConcurrentMessageQueue>();
        ...
        try
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                var buffer = new ConcurrentMessageQueue(
    ...
    public async Task MemoryPressure_ConcurrentBufferAccess()
    {
        ...
        try
        {
            ...
            {
                buffers.Add(new ConcurrentMessageQueue(
    ...
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("RaceWR"), slotCount: 8, slotSize: 64);
    ...
    private struct ExtendedTestSchema : IMemorySchema
    {
    ...
    private struct LargeArraySchema : IMemorySchema
    {
...
public class DataIntegrityExtremeTests
{
    ...
    public async Task TornRead_Detection_16ByteTypes()
    {
        ...
        using var memory = new StructuredMemory<GuidOnlySchema>(GetUniqueName("TornRead"), schema);
    ...
    public void CRC32_IntegrityAfterHeavyLoad()
    {
        ...
        using var buffer = new MemoryRegion(GetUniqueName("CRC32Load"), options);
    ...
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("PatternVerify"), slotCount: 1024, slotSize: 256);
    ...
    private struct GuidOnlySchema : IMemorySchema
    {
        public const string GuidField = "GuidValue";
...
public class BoundaryExtremeTests
{
    ...
    public void MPMC_MinimalSlots_Stress()
    {
        ...
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MinSlots"), slotCount: 2, slotSize: 64);
    ...
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("TwoSlotsFull"), slotCount: 2, slotSize: 64);
    ...
    {
        // Very small capacity
        using var buffer = new SingleProducerByteStream(GetUniqueName("MinCap"), 16);
    ...
    public void HPBuffer_ExactBoundaryAccess()
    {
        ...
        var options = new MemoryRegionOptions { Capacity = capacity };
        using var buffer = new MemoryRegion(GetUniqueName("ExactBound"), options);
    ...
    public void MPMC_MaxMessageSize_Repeated()
    {
        ...
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MaxMsg"), slotCount: 16, slotSize: slotSize);
