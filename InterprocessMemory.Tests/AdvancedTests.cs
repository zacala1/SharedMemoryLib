...
public class AdvancedTests
{
    ...
    [Test]
    public void SingleProducerByteStream_NullName_ShouldThrow()
    {
        Assert.Throws<ArgumentException>(() => new SingleProducerByteStream(null!, 1024));
    }

    [Test]
    public void SingleProducerByteStream_EmptyName_ShouldThrow()
    {
        Assert.Throws<ArgumentException>(() => new SingleProducerByteStream("", 1024));
    }

    [Test]
    public void SingleProducerByteStream_ZeroCapacity_ShouldThrow()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new SingleProducerByteStream(GetUniqueName("Test"), 0));
    }

    [Test]
    public void SingleProducerByteStream_NegativeCapacity_ShouldThrow()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new SingleProducerByteStream(GetUniqueName("Test"), -1));
    }

    [Test]
    public void SingleProducerByteStream_AccessAfterDispose_ShouldThrow()
    {
        var buffer = new SingleProducerByteStream(GetUniqueName("Dispose"), 1024);
        buffer.Dispose();
    ...
    [Test]
    public void MemoryRegion_NullName_ShouldThrow()
    {
        var options = new MemoryRegionOptions { Capacity = 1024 };
        Assert.Throws<ArgumentException>(() => new MemoryRegion(null!, options));
    }

    [Test]
    public void StructuredMemory_NullName_ShouldThrow()
    {
        var schema = new TestSchema();
        Assert.Throws<ArgumentException>(() => new StructuredMemory<TestSchema>(null!, schema));
    }
    ...
    [Test]
    public void SingleProducerByteStream_DataIntegrity_SequentialMessages()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("SeqIntegrity"), 64 * 1024);
    ...
    [Test]
    public void SingleProducerByteStream_PowerOf2Capacity_ShouldRoundUp()
    {
        // Request 1000 bytes, should round up to 1024
        using var buffer = new SingleProducerByteStream(GetUniqueName("PowerOf2"), 1000);
        Assert.That(buffer.Capacity, Is.EqualTo(1024));

        // Request 100 bytes, should round up to 128
        using var buffer2 = new SingleProducerByteStream(GetUniqueName("PowerOf2_2"), 100);
        Assert.That(buffer2.Capacity, Is.EqualTo(128));

        // Request exactly 512, should stay 512
        using var buffer3 = new SingleProducerByteStream(GetUniqueName("PowerOf2_3"), 512);
    ...
    [Test]
    public void SingleProducerByteStream_StressTest_HighThroughput()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("HighThroughput"), 1024 * 1024);
    ...
    [Test]
    public void SingleProducerByteStream_StressTest_WrapAround()
    {
        // Small buffer to force many wrap-arounds
        using var buffer = new SingleProducerByteStream(GetUniqueName("WrapStress"), 256);
    ...
    [Test]
    public async Task SingleProducerByteStream_SPSC_HighContention()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("SPSC_Contention"), 64 * 1024);
    ...
    [Test]
    public async Task MemoryRegion_ConcurrentReadWrite_WithLocks()
    {
        var options = new MemoryRegionOptions { Capacity = 4096 };
        using var buffer = new MemoryRegion(GetUniqueName("ConcurrentLock"), options);
    ...
    [Test]
    public async Task StructuredMemory_ConcurrentFieldAccess_WithLocks()
    {
        var schema = new TestSchema();
        using var memory = new StructuredMemory<TestSchema>(GetUniqueName("ConcurrentStrict"), schema);
    ...
    [Test]
    public void SingleProducerByteStream_EmptyWrite_ShouldSucceed()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("EmptyWrite"), 1024);
    ...
    [Test]
    public void SingleProducerByteStream_EmptyRead_ShouldReturnZero()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("EmptyRead"), 1024);
    ...
    public void MultipleDispose_ShouldNotThrow()
    {
        ...
        var buffer = new SingleProducerByteStream(GetUniqueName("MultiDispose2"), 1024);
        buffer.Dispose();
    ...
    private struct TestSchema : IMemorySchema
    {
    ...
    [Test]
    public void StructuredMemory_SchemaHash_ShouldBeStableAcrossInstances()
    {
        ...
        using var memory1 = new StructuredMemory<TestSchema>(uniqueName1, schema);
        using var memory2 = new StructuredMemory<TestSchema>(uniqueName2, schema);
    ...
    {
        var schema = new TestSchema();
        using var memory = new StructuredMemory<TestSchema>(GetUniqueName("DoubleLock"), schema);
    ...
    {
        var schema = new TestSchema();
        using var memory = new StructuredMemory<TestSchema>(GetUniqueName("DoubleReadLock"), schema);
    ...
    [Test]
    public void MemoryRegion_WriteLock_TimeoutDuringReaderWait_ShouldNotLeakLock()
    {
        var options = new MemoryRegionOptions { Capacity = 4096 };
        using var buffer = new MemoryRegion(GetUniqueName("LockLeak"), options);
    ...
    [Test]
    public void MemoryRegion_NullOptions_ShouldUseDefaults()
    {
        // Should not throw when options is null
        Assert.DoesNotThrow(() =>
        {
            using var buffer = new MemoryRegion(GetUniqueName("NullOptions"), null!);
            Assert.That(buffer.Capacity, Is.GreaterThan(0));
        });
    }

    [Test]
    public void ConcurrentMessageQueue_Statistics_FailedOperations_ShouldBeTracked()
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("FailStats"), slotCount: 2, slotSize: 64);
    ...
    [Test]
    public void MemoryRegion_ZeroLengthWrite_ShouldSucceed()
    {
        using var buffer = new MemoryRegion(GetUniqueName("ZeroWrite"), new MemoryRegionOptions { Capacity = 1024 });
    ...
    [Test]
    public void MemoryRegion_ZeroLengthRead_ShouldSucceed()
    {
        using var buffer = new MemoryRegion(GetUniqueName("ZeroRead"), new MemoryRegionOptions { Capacity = 1024 });
    ...
    [Test]
    public void MemoryRegion_WriteAtCapacityBoundary_ShouldSucceed()
    {
        const int capacity = 1024;
        using var buffer = new MemoryRegion(GetUniqueName("CapBoundary"), new MemoryRegionOptions { Capacity = capacity });
    ...
    [Test]
    public void MemoryRegion_WriteExceedingCapacity_ShouldThrow()
    {
        const int capacity = 1024;
        using var buffer = new MemoryRegion(GetUniqueName("ExceedCap"), new MemoryRegionOptions { Capacity = capacity });
    ...
    [Test]
    public void MemoryRegion_WriteSpanningBeyondCapacity_ShouldThrow()
    {
        const int capacity = 1024;
        using var buffer = new MemoryRegion(GetUniqueName("SpanBeyond"), new MemoryRegionOptions { Capacity = capacity });
    ...
    [Test]
    public void SingleProducerByteStream_ZeroLengthWrite_ShouldSucceed()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("LFZeroWrite"), 1024);
    ...
    [Test]
    public void SingleProducerByteStream_ZeroLengthRead_ShouldReturnZero()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("LFZeroRead"), 1024);
    ...
    [Test]
    public void SingleProducerByteStream_WrapAround_ShouldWorkCorrectly()
    {
        // Use small capacity to force wrap-around quickly
        using var buffer = new SingleProducerByteStream(GetUniqueName("LFWrap"), 64);
    ...
    [Test]
    public void SingleProducerByteStream_ExactCapacityWrite_ShouldSucceed()
    {
        const int capacity = 256; // Will be rounded up to power of 2
        using var buffer = new SingleProducerByteStream(GetUniqueName("LFExactCap"), capacity);
    ...
    [Test]
    public void SingleProducerByteStream_ExceedCapacityWrite_ShouldThrow()
    {
        using var buffer = new SingleProducerByteStream(GetUniqueName("LFExceedCap"), 256);
    ...
    [Test]
    public void ConcurrentMessageQueue_ZeroLengthWrite_ShouldThrow()
    {
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MpmcZeroWrite"), slotCount: 4, slotSize: 64);

        Assert.Throws<ArgumentException>(() => buffer.TryWrite(ReadOnlySpan<byte>.Empty));
    }

    [Test]
    public void ConcurrentMessageQueue_MaxSlotSizeWrite_ShouldSucceed()
    {
        const int slotSize = 256;
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MpmcMaxSlot"), slotCount: 4, slotSize: slotSize);
    ...
    [Test]
    public void ConcurrentMessageQueue_ExceedMaxMessageSize_ShouldThrow()
    {
        const int slotSize = 64;
        using var buffer = new ConcurrentMessageQueue(GetUniqueName("MpmcExceedMsg"), slotCount: 4, slotSize: slotSize);
    ...
    [Test]
    public void MemoryRegion_LargeNegativeOffset_ShouldThrow()
    {
        using var buffer = new MemoryRegion(GetUniqueName("NegOffset"), new MemoryRegionOptions { Capacity = 1024 });
    ...
    [Test]
    public void StructuredMemory_MaxLengthString_ShouldWork()
    {
        var schema = new TestSchema();
        using var memory = new StructuredMemory<TestSchema>(GetUniqueName("MaxStr"), schema);
    ...
    [Test]
    public void MemoryRegion_ConcurrentTimeout_AllShouldComplete()
    {
        using var buffer = new MemoryRegion(GetUniqueName("ConcTimeout"), new MemoryRegionOptions { Capacity = 1024 });
